...
void ft_udp_checksum(struct iphdr *ip_header, u_short *udp_payload) {
    u_long sum = 0;
    u_short tcp_len = ntohs(ip_header->tot_len) - (ip_header->ihl << 2);
...
port_state_t    do_udp_scan(int socket_fd, struct in_addr host_addr, uint16_t port, scan_type_t scan_type) {
...
port_state_t    do_tcp_scan(int socket_fd, struct in_addr host_addr, uint16_t port, scan_type_t scan_type) {
	struct sockaddr_in dst_addr = {AF_INET, port, host_addr, {0}};
	socklen_t dst_addr_len = sizeof(dst_addr);
